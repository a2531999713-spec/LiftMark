...
### 首页头像红点移除
- 移除首页右上角头像的 `showStatusDot` 状态点；该点仅表示登录状态（`authStatus === 'authenticated'`），并非未读消息，容易误导用户。`HomeHeader` 不再接收 `showStatusDot` 属性。

### 功能建议与问题反馈合并
- 账号面板（`AccountPanel`）主菜单中「功能建议」和「问题反馈」两行合并为单一「反馈与建议」入口。
- 点击后不再切换面板模式（不再有返回按钮的新界面感），改为弹出底部 `AppModalSheet`，用户可在弹窗内切换「功能建议 / 问题反馈」类型并填写内容。
- `app/profile/index.tsx` 同步合并两行为「反馈与建议」。
- `app/feedback.tsx` 独立页面更新为合并表单（类型选择 + 内容 + 联系方式），保留诊断信息。

### 首页标题滚动轮播
- `HomeHeader` 新增 `RollingTitle` 组件，标题文案每 8 秒淡入淡出轮播，从 `homeHeaderTitlePool` 中循环展示。
- 初始标题仍按日期哈希稳定选择，避免每次打开都从第一条开始。
- 间隔 8 秒不会过于频繁，避免干扰用户阅读。

### 训练小组成员删除与入口整理
- `MemberRepository` 接口新增 `deleteMember(memberId)`，`SQLiteMemberRepository` 实现软删除（`group_members` 和 `member_profiles` 同时设置 `deleted_at`）。
- 成员详情页（`app/member/[memberId].tsx`）新增删除按钮（danger 样式），带二次确认和所有者保护（`role === 'owner'` 不可删除）。
- 删除后入队同步（`pending_delete`），返回成员列表页。
- `app/(tabs)/members.tsx` 移除折叠式「联机小组能力」区块（邀请 / 加入 / 同步三个按钮入口混乱），替换为简洁提示卡，引导用户到账号面板「训练小组」入口。
...
- 首页 Header、计划卡、今日训练 Hero、当前小组开始训练卡和今日重点行高继续压缩，提升首屏内容密度。
- 右上账号面板继续缩小宽度、高度和行高，控制为头像附近展开的内部滚动面板。
- 账号面板主菜单删除“设置与反馈”，功能建议和问题反馈上移到反馈与法务分组。
- 编辑个人信息面板改为头像 + 昵称 / 手机号 / 练刻 ID 顶部布局，头像点击直接操作，头像旁不再展示说明或同步长文案。
- 性别字段改为点击展开选择，支持男 / 女 / 其他 / 不展示；未设置显示“选择性别”，设置后只显示当前结果。
- 云同步、数据备份、训练偏好、会员 / 激活码、功能建议、问题反馈改为直接展示内容，不再出现“查看详情 / 打开设置”中间按钮。
- 隐私政策和用户协议扩展为可阅读草案，不写虚假运营主体、备案号、客服电话或支付承诺。
- 首页加载增加数据库初始化等待、请求序号防 stale 更新、局部数据失败降级、空小组 / 无计划 / 无成员引导和真实错误日志。

## 2026-07-05 - compact-home-account-panel

- 首页首屏密度优化：头像缩小到 40dp，计划卡、今日训练 Hero、当前小组开始训练卡和今日重点列表整体压缩。
- 首页头部文案从按小时问候改为训练语境文案，展示今日计划 / 训练重点 / 日期 / 周次 / 当前小组。
- 首页右上角头像入口改为右上浮层账号面板，不再使用底部账号 Sheet。
- 账号面板主菜单取消独立“个人资料”入口，顶部头像 + 昵称摘要作为个人信息编辑入口。
- 个人信息编辑改为面板内完成，支持头像、昵称、年龄、性别、手机号、练刻 ID、会员和云同步状态。
- 资料草稿变更后仅在面板右上角显示保存按钮，保存成功后按钮消失，并同步账号缓存、登录会话和绑定成员名称。
- “切换小组”和“管理小组与成员”合并为“训练小组”入口，子状态内支持切换小组和进入成员管理 / 创建 / 设置。
- 新增 `account-panel-v3.png` 参考图和本轮 `design-qa.md` 验证记录。
- 验证：`npm run typecheck` 通过，`npm run lint` 通过，`npm test` 15 个套件 / 89 个用例通过；`npm run android:preview` 构建 APK 成功但 emulator package service 阻塞安装。

## 2026-07-05 - home-profile-account-entry

- 重构首页账号入口：首页右上角改为用户头像，点击打开轻量账号功能菜单。
- 底部 Tab 调整为首页 / 计划 / 记录，隐藏原“我的”Tab。
- 首页视觉结构调整为计划进度卡、今日训练 Hero、当前小组 + 开始训练合并卡、今日重点列表。
- 新增 / 重构个人资料页，分组承接账号资料、小组与同步、偏好与权益、法务与支持。
- 新增数据备份、切换小组、账号信息、反馈与法务草案页面入口。
- 修复昵称保存链路：账号资料缓存、登录 session 和当前账号绑定成员名称同步更新。
...
### 训练执行
- 小组训练轮换改为只按未完成 set 队列推进：同一组内 A/B/C 顺序完成，最后成员休息结束后回到 A 的下一组；最后一组必须所有成员完成后才进入下一动作。
- 休息面板展示当前休息成员、倒计时、已休息时间和下一位成员 / 下一组。
- 训练选项新增加做组：可为当前成员或当前动作所有成员追加未完成 set，不修改原计划。
- 重量步进统一支持 2.5kg 默认和 1.25kg 等自定义步进；已完成组编辑不再丢小数精度。

### 总结、图表与计划
- 训练总结 1RM 指标展示动作来源、重量和次数，并移除重复“再看一次总结”按钮。
- 计划执行趋势按周起始日期显示，每 7 天一个点；短数据靠左展示。
- 历史、动作详情、小组动作详情和训练分析 1RM 图表 X 轴使用真实日期 / 周起始日期，不再显示“第 N 次”。
- 计划页当前计划 Hero 和操作弹层新增明确的切换计划、编辑计划和主流计划库入口。

### 登录与文档
- 登录页保留手机号 / 练刻 ID 密码登录，并将验证码入口文案改为短信登录 / 注册；后端 `/auth/password/login`、`/auth/login-with-code`、`/auth/send-code` 已确认存在。
- 成员哑铃默认加重单位改为 2.5kg；已有成员自定义 2kg 或 1.25kg 不迁移覆盖。
...
### 个人记录与历史详情
- 个人历史页删除重复“训练查询”模块，保留动作筛选、趋势和当天记录；动作趋势按 session 聚合，个人入口只展示当前成员 sets。
- 训练详情右上角前置编辑入口，支持已完成记录新增动作、新增组、删组、基础信息编辑；个人口径新增/删组只作用于当前成员。
- 历史补录从单动作/统一组数改为多动作、多组独立重量和次数录入。
- 动作历史页删除平均 RPE 指标和 RPE 趋势图，保留容量、最佳重量和估算 1RM。

### 计划与今日训练
- 今日训练默认按当前计划已完成 session 自动跟随到下一个未完成训练日；用户手动选择周/日后保留手动选择。
- 计划首页删除重复的“我的计划 / 计划操作 / 计划库”主页面区块，低频操作收进顶部更多菜单。
- 计划详情右上角改为操作菜单，不再跳回计划页；创建页支持多训练日创建和通过 `editPlanId` 编辑用户计划。
- `PlanRepository.updateUserPlan()` 新增替换式更新用户计划结构，系统方案仍只读。

### 登录、成员与文案
- 移动端密码登录改用 `/auth/password/login`，支持手机号 / 练刻 ID；后端保留 `/auth/login` 兼容旧客户端。
- 登录页增加密码显示切换，验证码入口文案改为短信登录 / 注册。
- 成员新增失败文案从“成员表单出错”改为“成员信息需要调整”。
- 普通 App 页面移除“本机 / 本地”技术化文案，改为“当前设备 / 训练记录 / 小组”等用户语义。

### 验证
- 已通过：`npm run typecheck`、`npm run lint`、`npm test -- --runInBand`。
- 后端已通过：`npm run typecheck`、`npm run build`。
