...
- 首页头部主文案来自短训练文案池，按日期稳定选择；不要把今日计划标题、训练重点或按小时问候放到左上角。
- 首页计划卡、训练 Hero、当前小组开始训练卡和今日重点列表已压缩；当前小组卡不承载管理成员或邀请成员职责。
- `AccountPanel` 主菜单顶部头像 + 昵称摘要就是个人信息入口；不要再新增单独“个人资料”行。
- 个人信息编辑在面板内完成，头像 + 昵称 / 手机号 / 练刻 ID 在顶部，头像点击直接操作；头像旁不要放“点击头像直接更换”或同步长说明。
- 性别字段是折叠选择：未设置显示“选择性别”，点击后展开男 / 女 / 其他 / 不展示，选择后只显示当前结果。
...
- 云同步、数据备份、训练偏好、会员 / 激活码、功能建议和问题反馈都在 `AccountPanel` 子状态内直接显示内容；不要再加“查看详情 / 打开设置”二次按钮。
- “设置与反馈”主入口已删除；功能建议和问题反馈在账号面板上一级。
- 隐私政策和用户协议入口保留为 `/legal/privacy` 与 `/legal/terms`，当前是可阅读草案，正式上线前补实际运营主体与服务细节。
- 首页加载已经改为等待 `initializeLocalDatabase()`、复用初始化 Promise、防 stale setState，并让头像同步 / 历史详情等局部失败降级，不应再因局部请求失败卡住整页。
