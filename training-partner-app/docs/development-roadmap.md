...
## 2026-07-21 v2.11.1 训练写入与结束稳定性

- 已完成最新 set patch 合并、批量 Repository、原子 session 完成和同步候选恢复。
- 已完成结束后报告/progression/achievement/sync 调度、统一执行阶段与同步 ref 锁。
- 已修复保存退出、真实进度、自重 0kg 和多人批量调整语义。
- NE2210 隔离 15/64/200 set 原子结束均满足目标；完整 188 UI/断网恢复流程仍需人工验收。
- 无 SQLite/PostgreSQL migration、API 或本轮服务器部署；下一步先验收并部署遗留 v2.11.0 服务端，再开始新架构阶段。

## 2026-07-21 v2.11.0 训练连续性与成就中心

- 已完成 11 项稳定成就目录、有效训练/组口径、周一自然周连续性和确定性本地引擎。
- 已完成 SQLite 账号级少量聚合、离线优先、远端单调合并与账号级 seen codes。
- 已完成首页连续性卡、`/achievements`、我的入口和训练总结后的非阻塞解锁 Sheet。
- 已修复 API 指标口径、N+1、首次 `achieved_at` 保留与幂等 seed。
- 无 SQLite/PostgreSQL migration；API 与 seed 合并后需要部署。
- 延期：排行榜、积分/商城、小组挑战、公开成就、PR 成就和动作图标。

## 2026-07-15 v2.10.0 恢复状态评估与当日训练调整

- 已完成六项评估、确定性评分、连续低状态、每日幂等 Repository 与真实日期趋势。
- 已完成今日页异步状态卡、未评估可跳过、小组成员状态与明确确认的 session 级建议。
- 已完成 A/B/C 内存过滤和当前 session 未完成组 7.5% 默认降重；计划、历史和 progression 均不回写。
- 沿用现有 `recovery_logs` 与同步 contract，无 migration、API 或服务器部署。
- 延期：每成员独立动作结构、恢复状态写入报告、穿戴设备、AI 调计划、动作图标。

## 2026-07-15 v2.9.0 推荐计划库与完整详情

- 已完成独立计划库、四类筛选、搜索、推荐/全部可用/不可用分区。
- 已完成 8 套系统方案真实 phase/week/day/exercise 详情、未知动作和元数据降级、确定性时长区间。
- 已完成已有副本默认复用、明确重复复制、双击锁、复制后激活并确保周期。
- 已完成首次引导全屏详情且保留草稿；计划页无有效安排时提供系统方案/创建/导入三个直接入口。
- 无 migration、API 或服务端部署；动作图标、进阶建议和 v2.7.2 延期项不在本轮。

## 2026-07-11 plan-cycle-report-history-sprint

- 已完成：训练报告详情页、训练结束与历史详情双入口、旧记录无报告只读 fallback。
- 已完成：当前周期卡、周期总结、手动完成/归档确认和幂等 summary 重算。
- 已完成：全部、当前周期、具体历史周期、自由训练、补录训练筛选及账号/小组作用域重置。
- 已完成：typecheck、lint、36 套件/177 测试和 Android arm64 Release 构建。
- 待完成：在隔离设备使用 188 做写入全流程，并对 176 做只读数据隔离验收；本项完成前不标记为可发布。
- 主动延期：自动下一周期、AI 总结、智能调计划、动作进步预测、复杂周期图表。

## 2026-07-05 compact-home-account-panel-sprint

- 已完成：底部主导航保持首页 / 计划 / 记录，`settings`、`members`、`explore` 继续作为隐藏路由，不作为底部 Tab 展示。
- 已完成：首页右上角 40dp 头像成为账号入口，点击后从头像附近展开右上账号控制面板，不再使用底部账号 Sheet。
- 已完成：首页重组并压缩为训练语境头部、计划进度卡、今日训练 Hero、当前小组 + 开始训练合并卡和今日重点列表，首屏能露出更多核心训练内容。
- 已完成：首页头部文案从机械问候改为短训练文案池，按日期稳定选择；今日计划日和训练重点只在计划卡 / Hero 中展示。
- 已完成：账号面板主菜单删除独立“个人资料”行，顶部头像 + 昵称摘要即个人信息入口。
- 已完成：个人信息编辑改为面板内子状态，头像 + 昵称 / 手机号 / 练刻 ID 顶部布局，头像直接触发操作，不再放说明文案。
- 已完成：性别字段改为点击展开选择男 / 女 / 其他 / 不展示，未设置时显示“选择性别”，设置后只显示当前选择。
- 已完成：保存按钮只在资料草稿变更后出现在右上角；保存后同步账号缓存、登录会话和绑定成员名称并隐藏保存按钮。
- 已完成：“切换小组”和“管理小组与成员”合并为“训练小组”入口，面板子状态可切换当前小组并进入管理 / 创建 / 设置。
- 已完成：云同步、数据备份、训练偏好、会员 / 激活码、反馈与建议直接进入内容面板，不再出现查看详情式二次按钮。
- 已完成：删除“设置与反馈”主入口，反馈与建议上移到账户面板主菜单；功能建议和问题反馈合并为单一入口，点击后弹出底部弹窗。
- 已完成：首页右上角头像红点移除（原仅表示登录状态，非未读消息）。
- 已完成：首页标题改为滚动轮播，每 8 秒淡入淡出切换。
- 已完成：训练小组成员删除功能（软删除 + 所有者保护 + 同步入队）。
- 已完成：成员列表页移除折叠式「联机小组能力」区块，入口归入账号面板「训练小组」。
- 已完成：隐私政策和用户协议扩展为可阅读草案，正式上线前再补实际运营主体、联系方式和合规信息。
- 已完成：首页加载增加请求序号防 stale 更新，头像同步和历史详情失败降级，空小组 / 无计划 / 无成员走引导态。
- 后续：真机确认右上面板定位、Android 返回键子状态回退、头像选择权限流和法务正式文本。

## 2026-07-03 group-member-identity-avatar-sync-sprint

- 已完成：`group_members` 本地 schema 增加 `user_id`、`member_type`、`local_member_id` 和 `joined_at`，区分真实成员与本地成员。
- 已完成：邀请码加入后拉取云端小组成员并切换当前小组；成员页展示加入小组、邀请成员、真实成员 / 本地成员标识。
- 已完成：账号头像上传/删除同步云端用户头像与真实成员 profile，成员列表、训练页、记录页统一解析相对头像 URL。
- 已完成：训练结束上传待确认数据只允许选择真实成员；接受待确认数据后写入云端通用 workout 表并落本地 SQLite 历史。
- 后续：补齐更完整的端到端云同步回归、待确认数据过期清理和二维码加入。
...
- 已完成：折线图比例、绘图区 padding、Y 轴刻度、单位、空状态和周起始日期标签修复。
...
## 10. 2026-07-02 训练执行页修复状态

- 已完成：训练执行页顶部固定“结束训练”；稳定队列修复小组轮换；休息面板改为状态提示；本次替换、加做、跳过和临时动作只影响当前 session；总结页询问是否同步到用户计划。
- 已完成：默认重量步进改为 2.5kg 并补充小数重量测试；启动路径将本地 seed 后台化并对远程 auth 校验增加短超时兜底。
...
已完成每动作独立处方编辑、范围次数、固定重量/%1RM、休息和备注；后续可在不修改历史 workout snapshot 的前提下扩展批量处方调整。动作图标与媒体因素材和目的地未定而延期。
