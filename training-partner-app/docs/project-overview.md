...
- 当前外层目录仍为 `C:\Users\zhw\Documents\LiftOn`，仅作为本地目录尚未手动改名的路径保留；代码和文档语义已迁移到 LiftMark。
...
本项目品牌为“练刻 / LiftMark”，是一个安卓优先、iOS 预留的多人力量训练 App。产品目标是替代 Excel 训练计划表，让 2-4 人在健身房训练时快速查看今日训练、自动计算每个人建议重量、记录实际完成情况，并在训练后生成下次训练建议。
...
| 功能 | MVP 状态 | 说明 |
|---|---|---|
| 多成员管理 | 计划内 | 支持 2-4 人，成员独立 1RM 和加重单位 |
...
当前处于稳定性与基础体验 Sprint：品牌中文化、APK 一键预览、设置页基础功能、计划 `.liftmark` 导出 service 和历史推算基础逻辑已完成；完整业务持久化烟测仍待后续人工验证。
