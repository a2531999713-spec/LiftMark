...
11. 当前组输入先本地乐观更新，并以 450ms 防抖调用 `WorkoutRepository.saveSet`；用户点击“完成本组”时会合并未落库输入和完成状态，一次保存完整组数据。
12. 用户完成一组后，如存在计划休息时间，进入休息面板并显示倒计时、建议休息、已休息和已恢复状态；倒计时结束写入实际休息秒数，但不推进下一组。
13. 用户可通过“本次调整”替换当前动作、加做一组、跳过当前动作或添加临时动作，也可通过顶部“结束训练”完成训练。
...
- 2026-07-02 起，休息倒计时结束只展示“已恢复”提示并保存实际休息秒数，不显示“开始下一组”或“提前开始下一组”，也不推进执行光标。
- 训练执行页顶部右侧保持“结束训练”；替换、加做、跳过和临时添加动作统一放在“本次调整”入口。
- 本次调整只写当前 `workout_exercise_records` / `workout_sets`，通过备注标记 `本次替换`、`加做组`、`本次跳过动作` 或 `临时添加动作`，不直接修改 `plan_exercises`。
- 2026-07-07 起，训练执行页不再把每一次输入事件立即写库；完成本组会消费待保存草稿，避免快速点击时保存旧重量/次数。
- 添加或移除参与成员后只增量更新当前训练快照，不整页重载。
