...
## 2026-07-03 待确认训练数据补充

- 训练结束后，记录者可把其他真实成员的训练组上传到待确认队列；目标成员必须是同组且有 `userId` 的真实成员。
- 本地成员继续保留在本机训练记录中，但不会作为跨设备上传目标。
- 目标用户接受后，云端写入该用户 workout 同步表，移动端同时把回传的 session/sets 写入本地 SQLite 历史。
...
## 10. 2026-07-02 执行页当前边界

- 训练执行 cursor 由 workout domain 队列派生，UI 休息状态不决定下一组。
- 休息面板只展示倒计时和已恢复状态，不提供开始下一组按钮。
- 本次替换、加做、跳过和临时动作默认只影响当前 session；是否同步到用户计划由训练总结页二次确认。
