...
## 2026-07-03 小组邀请与真实成员补充

- 小组成员支持邀请码加入；加入后云端 `group_members.user_id` 绑定真实账号，移动端同步为 `memberType='real'`。
- 当前设备手动创建的成员仍保留为 `memberType='local'`，只用于本机训练轮换。
- 成员页提供“加入小组”和“邀请成员”入口，邀请码可复制或系统分享。
...
## 10. 2026-07-02 训练执行关联

- 小组训练执行顺序由 workout 模块的稳定队列控制，成员顺序只作为同一动作同一组内的排序依据。
- 休息倒计时结束不会自动切换成员或组；小组页和成员状态不应写入训练执行 cursor。
- 总结页按本次 session sets 统计参与成员，未参与成员仍不进入训练执行和总结数据。
