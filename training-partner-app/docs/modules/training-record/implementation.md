...
## 2026-07-11 补充：周期筛选与完整报告

- 记录 Tab 已改用 `HistoryRepository.listHistory()` 聚合摘要，支持当前周期、具体历史周期、自由训练和补录筛选，不逐条调用详情。
- 历史详情和训练结束页都可进入完整训练报告；旧训练无 report 时只读降级，不自动补写。
- 筛选状态绑定 owner + group scope，账号或小组变化时重新解析并重置。
