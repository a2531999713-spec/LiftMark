...
## 2026-07-03 真实成员与本地成员补充

- `GroupMember` 区分 `real` 和 `local`：真实成员通过注册账号和邀请码加入小组，绑定 `userId`；本地成员由当前设备手动创建，不绑定账号。
- 本地成员继续支持本机多人轮换和历史记录，但不会被作为跨设备待确认训练数据的上传目标。
- 成员列表和设置成员列表会标注“真实成员 / 本地成员”；上传成员页只展示真实成员。
- 账号头像更新后同步当前账号在本地所有小组里的真实成员 profile；远程头像 URL 支持 `/uploads/...` 相对路径解析。
...
- 账号头像与成员头像分表保存；训练相关页面只读取成员头像。账号头像更新后会同步当前账号绑定的真实训练成员头像。
...
- 成员身份：真实成员绑定 `userId`，本地成员保留 `localMemberId`。
