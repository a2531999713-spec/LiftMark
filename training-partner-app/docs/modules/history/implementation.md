...
## 2026-07-21 补充：成就事实口径

- 成就只复用账号范围内已保存的训练事实，不向历史 session/set 回写任何状态。
- completed workouts 与 12 周节奏仅包含未软删、completed 且至少有一组有效完成 set 的 session。
- 容量只累计 completed、未 skipped、未软删组；训练历史中的重量和单位保持原样，显示层按用户单位转换。
- 已解锁状态和首次日期不影响历史筛选、周期报告或个人记录计算。
...
5. 2026-07-02 起，训练中替换、加做、跳过和临时动作都按 session 快照进入历史统计；计划同步只影响未来计划，不回写历史。
