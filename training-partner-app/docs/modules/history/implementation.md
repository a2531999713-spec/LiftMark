...
- `app/(tabs)/history.tsx` 的个人视图结构为：本周概览、核心指标、动作筛选、训练趋势、日历当天记录、训练查询。
- 个人记录页不再以近期训练列表作为顶部核心模块；列表命名为“训练查询”，服务于检索和进入详情。
...
- 训练分析页支持最近 4 / 8 / 12 周。
- 新增 `app/history/exercise/[exerciseId].tsx`：展示当前成员单动作历史、容量、最佳重量、估算 1RM、RPE 趋势和最近有效组。
