...
| 文件 | 说明 |
|---|---|
| `app/(tabs)/history.tsx` | 训练历史页，展示近期训练和基础趋势卡片。 |
...
## 10. 2026-07-02 训练调整展示边界

- 历史记录继续以 session 快照为准，替换动作按实际 `workout_exercise_records.exercise_id` 统计，同时保留 `replaced_from_exercise_id`。
- 加做组、跳过动作和临时动作是当前 session 的训练事实；历史页不得回读当前计划来覆盖这些记录。
- 总结页同步到用户计划后，只影响后续训练计划，不重写已完成历史。
