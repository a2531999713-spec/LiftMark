...
- 计划页新增当前/最近周期卡，周期总结路由为 `/plan/cycle/[cycleId]`。
- `PlanRepository` 支持 scoped overview、完成、summary 重算和归档；完成/归档 UI 均二次确认，历史训练和报告不删除、不重绑。
- 重复重算/完成/归档复用既有 owner + cycle summary；归档周期不再作为 active 返回，但仍可从历史周期筛选访问。
