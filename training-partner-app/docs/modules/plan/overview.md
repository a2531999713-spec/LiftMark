...
## 2026-07-15 推荐计划库与完整详情

- 独立计划库支持目标、频率、水平、器械筛选与搜索；计划页/探索页进入同一详情路由。
- 详情从系统模板读取真实阶段、周、训练日、动作处方和时长区间；缺数据时显示元数据降级。
- 系统方案保持只读；复制用户副本后立即激活并保证周期。已有副本默认复用，重复复制必须确认。
- 计划页无训练日时直接展示“使用系统方案 / 创建计划 / 导入计划”。
