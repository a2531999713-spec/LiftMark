...
## 2026-07-10 导入权限与可用计划边界

- `import_plan` 本身只要求登录，不属于 Pro 专属功能；真正写入导入计划前，才按可用用户计划数检查 `create_plan` 上限。
- 可用用户计划必须当前账号可见、未软删除、非系统模板，并且至少有 1 个训练日和 1 个计划动作。
- 旧空计划不自动删除，只在“我的计划”、免费版数量和活动计划选择中过滤。
- 计划页在无可执行当前计划时直接显示“使用系统方案 / 创建计划 / 导入计划”；三条路径成功后都立即设为当前计划。
- 当前创建页仍使用组件内草稿，只有点击保存才在事务中写入计划模板、阶段、训练日和计划动作。

## 2026-07-08 计划消失与训练数据失联修复 + 管理弹窗打不开修复

- 重大 bug 修复：导入计划时保留导出文件中的原 plan ID（不再强制生成新 ID），并在 `importUserPlan` 事务内检测 ID 冲突，确保重装后重新导入计划能与旧训练记录的 `plan_id` 重新关联。
- 管理计划弹窗打不开修复：移除 `manageContent` 的 `flex: 1`（在 `AppModalSheet` 普通 View 父容器中导致 ScrollView 高度塌缩为 0）。

## 2026-07-08 管理面板滚动与内联更多操作 + Tab 切换白屏修复

- 管理计划弹窗内容支持滚动，解决计划数量多时超出屏幕的问题；「更多操作」改为内联展开两个按钮，不再弹独立界面。
- Tab 切换白屏修复：load 函数已有数据时跳过 loading，渲染保留旧数据不卸载子树，loading 改为仅在无数据时显示 overlay。
- 编辑计划页顶部间距修复：`Screen` 改为 `safeTop={false}`，避免与 Stack header 双重顶部空间叠加。

## 2026-07-08 管理计划面板重设计与最近训练图表改造

- 管理计划弹窗按用户设计方案重构：「我的计划」+ ⋮ 更多操作（新建 / 导入）+「探索更多」（前 3 个系统方案 + 查看全部）；当前计划高亮 ⭐，非当前计划提供设为当前 / 编辑 / 分享 / 删除按钮。
- 计划页「最近执行」改名「最近训练」，数据源从「按自然周聚合最近 4 周」改为「最近 6 个完成训练 session」，避免周一没数据时图表空白。
- `MiniBarLineChart` 默认不显示 Y 轴刻度，数值改为点击柱子或折线点后才显示，柱子和折线点选中时高亮。

## 2026-07-07 编辑计划页面与仪表盘重设计

- 编辑计划页改为单行紧凑概览 + 周几选择器 + 训练日 / 动作长按拖拽排序 + 复制 / 添加图标 + 嵌套滚动；返回退出时若有未保存修改会提示「放弃修改 / 继续编辑」。
- 计划页「本周执行」改名为「最近执行」，图表从单一折线图改为柱状图+折线图组合（柱=训练量、折线=完成训练次数），StatTile 缩小，去除「上一周 / 下一周」按钮。
- 当前计划卡顶部右上角改为可点击的设置图标，点击弹出管理计划弹窗；原「管理计划 / 编辑当前计划」两个按钮收回管理弹窗。
...
## 9. 2026-07-02 训练总结同步边界

- 训练执行中的本次调整先写入 session，不直接改计划。
- 总结页可把替换、加做、跳过和临时动作同步到当前用户计划，底层调用 `PlanRepository.updateUserPlan()`。
- 系统方案和系统来源计划不可直接写回；需要先复制为用户计划或编辑用户计划副本。
