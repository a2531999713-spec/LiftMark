...
- 小组成员系统重构：真实成员通过邀请码加入并绑定 `userId`，本地成员继续保留但不能冒充真实成员。
- 头像全局同步修复：账号头像同步云端成员 profile，移动端统一解析相对头像 URL。
- 待确认训练数据：上传目标只允许真实成员，接受后写入云端同步表和本地 SQLite 历史。
- SQLite migration v12 和 API migration 006 已同步到数据库、架构、成员、小组、训练和专项文档。
...
- 训练休息面板改为状态提示并保存 `actual_rest_seconds`；无建议重量的新动作可直接输入并完成，后续组 / 后续 session 可复用最近实际重量。
...
- 日期：2026-07-02
- 报告：训练执行页 cursor、休息状态、本次调整、训练总结同步到计划、认证启动兜底、seed 启动优化和 Repository API 已同步到相关文档。
