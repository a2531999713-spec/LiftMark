...
export default function TodayRoute() {
  ...
  const loadHome = useCallback(async () => {
    ...
    try {
      ...
      const [dashboardSnapshot, nextPhasesInitial, nextPlanDaysInitial, nextAccountProfile] = await Promise.all([
        loadOrDefault(
          'home dashboard snapshot load',
          ...
          }),
          {
            ...
            weeklyOverview: emptyWeeklyOverview,
          },
        ),
        nextActivePlan
      ...
      if (nextActivePlan) {
        ...
        const completedPlanDayKeys = new Set(
          dashboardSnapshot.completedPlanDayKeys.filter((key) => key.startsWith(`${nextActivePlan.id}:`)),
        );
      ...
      setAccountProfile(nextAccountProfile);
      setLastPerformanceByExerciseId(dashboardSnapshot.lastPerformanceByExerciseId);
      setPlanPhases(nextPhases);
      setExerciseMap(nextExerciseMap);
      setRecentVisibleSessionCount(dashboardSnapshot.recentVisibleSessionCount);
      setWeeklyOverview(dashboardSnapshot.weeklyOverview);
      try {
        if (currentMember) {
