...
- 高频修改重量、次数、RPE 或备注后，结束训练不再等待此前每一次输入产生的历史保存任务。
- 完成本组、多人加减组和结束训练改为批量本地保存；离线时仍可可靠完成训练。
- “保存并退出”会保留可继续的训练，“结束训练”才生成正式结果；两者不再混用。
- 动作进度由真实完成/跳过状态计算，自重动作可以按 0kg 正常记录。
- 报告、进阶建议、成就和云同步在总结页打开后继续处理，不拖慢关键路径。

## v2.11.0 - 2026-07-21

### 训练连续性与成就中心

- 首页新增紧凑的“训练连续性”卡片，可查看本周训练、连续活跃周和下一里程碑。
- 新增完整成就中心，展示最近 12 周节奏、下一里程碑、进行中与已解锁成就。
- 训练完成后若达到新里程碑，会在总结页显示一次轻量提示；不会拖慢保存或总结页打开。
- 连续性以“每个自然周至少完成一次有效训练”为准，不要求每天训练。
- 成就可离线计算；恢复网络后会与云端安全合并，进度和首次解锁时间不会倒退。
- 本版不加入排行榜、积分商城、小组挑战或强制分享。

## v2.10.0 - 2026-07-15

### 恢复状态与今日调整

- 训练前可用 20 秒记录睡眠、食欲、训练欲望、肌肉酸痛、关节不适和整体疲劳。
- 今日状态会给出按原计划、减少辅助动作、临时降重、只做主项或优先恢复等建议。
- 建议必须由用户确认，只作用于本次新训练；休息建议下仍可二次确认按原计划训练。
- 小组训练会显示成员今日状态，未评估成员明确标记为“未评估”。
- 最近记录按真实日期展示趋势，不把未记录日补成 0。

### 数据边界

- 恢复记录本地优先、按账号/小组/成员隔离，并进入既有同步队列。
- 不修改训练计划、历史记录或长期进阶建议，不属于医疗诊断。
- 本版没有数据库迁移、API 或服务器部署变更。
...
- 可按训练目标、每周频率、训练水平和器械条件筛选，也可搜索名称与标签。
- 计划详情展示真实训练阶段、训练日、动作顺序、组次、强度、休息、备注与合理时长区间。
- 已复制方案优先打开并激活现有副本；“再复制一份”需要明确确认。
- 首次引导中的“查看计划详情”不会清空已填写资料。

### 数据与验证

- 系统模板保持只读；本次不修改数据库结构、云端 API 或服务器。
- `npm run typecheck`、`npm run lint` 和完整 Jest 已通过；Android arm64 Release 构建结果见本次提交最终报告。
